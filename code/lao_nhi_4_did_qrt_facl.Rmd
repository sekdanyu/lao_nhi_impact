...
data_all <- data %>%
  ...
  summarise_at(c("opo5", "opu5", "ipo5", "ipu5", "ipday",
               ...
               sum, na.rm = TRUE) %>%
  ungroup() %>%
  mutate(lopo5  = ifelse(opo5  > 0, log(opo5),  NA_real_),
         lopu5  = ifelse(opu5  > 0, log(opu5),  NA_real_),
         lipo5  = ifelse(ipo5  > 0, log(ipo5),  NA_real_),
         lipu5  = ifelse(ipu5  > 0, log(ipu5),  NA_real_),
         lipday = ifelse(ipday > 0, log(ipday), NA_real_),
         ldel   = ifelse(del   > 0, log(del),   NA_real_)) %>%
  ...
  mutate(treat = ifelse(group == max(group), 0, treat),
         ...
         dnum2 = as.numeric(dnum))

head(data_all)

# 1210 Khounkham included
...
data_hos <- data %>%
  filter(quarter >= 2015.4 & quarter <= 2017.3,
         province != "01 Vientiane Capital",
         factype == "hos",
         district != "1210 Khounkham") %>%  # drop unbalanced data
  mutate(treat = ifelse(group == max(group), 0, treat),
         group = ifelse(group == max(group), 0, group),
         ipo5 = if_else(district == "0202 Mai", NA_real_, ipo5),
         dnum2 = as.numeric(dnum))
head(data_hos)

# 1210 Khounkham: DH newly built in 2017 -> dropped
# 1600 Champasak: ipday NA
# 0202 Mai: ipo5 is zero (due to deduction) -> change to NA
...
# 1210 DH Khounkham: DH newly built in 2017 -> dropped
# 0202 Mai: ipo5 is zero (due to deduction) -> change to NA
...
# ------------------------------------------------------------------------------
# Health Center (HC) - aggregated at the district level
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Health Center (HC) - individual facilities
# ------------------------------------------------------------------------------

if(!dir.exists(paste0(outpath, "fac"))) {dir.create(paste0(outpath, "fac"))}

data.q.fac <- read_excel("./data export/02_clean_7_q_fac.xlsx") # district
warnings()
str(data.q.fac)
...
data_fac <- data.q.fac %>%
  ...
  mutate(treat = ifelse(group == max(group), 0, treat),
         group = ifelse(group == max(group), 0, group),
         dnum2 = as.numeric(dnum),
         dnum = as.numeric(factor(faccode)))
...
# ------------------------------------------------------------------------------
# Combine
data.flv <- list(data_all, data_hos, data_dh, data_hc, data_fac)
...
covariate_fac <- ~1
covariate <- list(covariate_all, covariate_hos, covariate_dh, 
                  covariate_hc, covariate_fac)
...
vc.cls.d <- att_gt(yname = outname[i], tname = "time", idname = "dnum",
                 gname = "group", control_group = "notyettreated",
                 base_period = "varying",  # varying reference point
                 est_method = "dr", clustervars = "dnum2",
                 xformla = covariate[[h]], panel = TRUE, bstrap = TRUE, 
                 cband = TRUE, data = data.flv[[h]])
glance(vc.cls.d)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
...
listv <- c("v", "v.cls", "vc", "vc.cls.p", "vc.cls.d")
attgt_list_v <- lapply(listv, get)

listu <- c("u", "u.cls", "uc", "uc.cls.p", "uc.cls.d")
