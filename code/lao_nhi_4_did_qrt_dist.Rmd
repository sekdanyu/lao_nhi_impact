...
packageVersion("DRDID")         # 1.3.0
packageVersion("did")           # 2.5.1
packageVersion("fixest")        # 0.14.2
packageVersion("did2s")         # 1.2.1
packageVersion("etwfe")         # 0.6.2
packageVersion("bacondecomp")   # 0.1.1
packageVersion("HonestDiD")     # 0.2.8
...
# Number of health facilities at 2015.4
data %>%
 filter(quarter %in% c("2015.4")) %>%
 group_by(faclevel) %>%
 summarise(count = n_distinct(district),
           sum = sum(facnum, na.rm = TRUE))
# 148 districts
# (134 district with district hospitals + 14 districts without district hospital)
# (1017-134+100 = 983 health centers)
...
# data <- data %>% 
#   left_join(data.zero %>% 
#               dplyr::select(dnum, time, opo5r.zero, opu5r.zero, ipo5r.zero, 
...
# https://github.com/asheshrambachan/HonestDiD (Callaway and Sant’Anna)
#' https://github.com/pedrohcgs/CS_RR


# ------------------------------------------------------------------------------
# Treatment effects at a selected event time (default: e = 0)
# ------------------------------------------------------------------------------

#' @title honest_did_cluster_boot
#'
#' @description a function to compute a sensitivity analysis
#'  using the approach of Rambachan and Roth (2021)
#'
#' @param ... Parameters to pass to the relevant method.
honest_did_cluster_boot <- function(...) UseMethod("honest_did_cluster_boot")

#' @title honest_did_cluster_boot.AGGTEobj
#'
#' @description a function to compute a sensitivity analysis
...
honest_did_cluster_boot.AGGTEobj <- function(es,
                                e          = 0,
  ...
  es_inf_func <- as.matrix(es$inf.function$dynamic.inf.func.e)
  
  n <- nrow(es_inf_func)
  K <- ncol(es_inf_func)
  
  # ------------------------------------------------------------
  # Province-cluster multiplier-bootstrap covariance matrix
  
  boot <- did::mboot(
    inf.func  = es_inf_func,
    DIDparams = es$DIDparams,
    pl        = FALSE,
    cores     = 1,
    return_V  = TRUE
  )
  
  # Number of province clusters
  cluster_vector <- es$DIDparams$cluster_vector
  
  if (is.null(cluster_vector)) {
    stop("cluster_vector is missing from es$DIDparams.")
  }
  
  G <- length(unique(cluster_vector))
  
  # Identify non-degenerate IF columns in the same spirit as did::mboot()
  ndg <- (!is.na(colSums(es_inf_func))) &
         (colSums(es_inf_func^2) >
            sqrt(.Machine$double.eps) * 10)
  
  # Bootstrap covariance on the estimator scale
  V_nd <- boot$V * G / n^2
  
  if (sum(ndg) != ncol(V_nd)) {
  stop("Mismatch between non-degenerate IF columns and bootstrap covariance matrix.")
  }
  
  # Reconstruct full K x K covariance matrix.
  # The universal reference period has a zero row/column.
  V <- matrix(
    0,
    nrow = K,
    ncol = K
  ...
  post_att <- as.numeric(t(baseVec1) %*% beta[(npre + 1):(npre + npost)])
  ...
  return(list(robust_ci=robust_ci, orig_ci=orig_ci, 
              point_est = post_att, type=type,
              V = V, n_clusters  = G, n_units = n))
}

# ------------------------------------------------------------------------------
# Province-cluster robust + Bootstrapping + Avg of post-treatment periods
# ------------------------------------------------------------------------------

#' @title honest_did_cluster_boot_avg
#'
#' @description a function to compute a sensitivity analysis
#'  using the approach of Rambachan and Roth (2021)
#'
#' @param ... Parameters to pass to the relevant method.
honest_did_cluster_boot_avg <- function(...) UseMethod("honest_did_cluster_boot_avg")

#' @title honest_did_cluster_boot_avg.AGGTEobj
#'
#' @description a function to compute a sensitivity analysis
#'  using the approach of Rambachan and Roth (2021) when
#'  the event study is estimating using the `did` package
...
#' @param ... Parameters to pass to `createSensitivityResults` or
#'  `createSensitivityResults_relativeMagnitudes`.
honest_did_cluster_boot_avg.AGGTEobj <- function(es,
                                type       = c("smoothness", "relative_magnitude"),
                                gridPoints = 500,
                                ...) {
  ...
  post_att <- as.numeric(t(l_vec) %*% beta[(npre + 1):(npre + npost)])
  ...
  return(list(robust_ci=robust_ci, orig_ci=orig_ci, 
              point_est = post_att, type=type, l_vec = l_vec,
              V = V, n_clusters  = G, n_units = n))
}
...
data_g9 <- data %>% 
  ...
  mutate(treat = ifelse(group == max(group), 0, treat),
         group = ifelse(group == max(group), 0, group))
head(data_g9)
...
outname <- c(
  ...
  # log-count
  "lopo5", "lopu5", "lipo5", "lipu5", "lipday", "ldel"
)
...
# w/ covariates, SE clustered at provincial level, no bootstrap
vc.cls.p.noboot <- att_gt(yname = outname[i], tname = "time", idname = "dnum",
                 gname = "group", control_group = "notyettreated",
                 base_period = "varying",  # varying reference point
                 est_method = "dr", clustervars = "pnum",
                 xformla = covariate, panel = TRUE, bstrap = FALSE, 
                 cband = FALSE, data = data_g9)
glance(vc.cls.p.noboot)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- vc.cls.p.noboot", "- att")) %>% put()
vc.cls.p.noboot %>% put()
warnings()

# w/ covariates, SE clustered at provincial level 
vc.cls.p <- att_gt(yname = outname[i], tname = "time", idname = "dnum",
                 gname = "group", control_group = "notyettreated",
                 base_period = "varying",  # varying reference point
                 est_method = "dr", clustervars = "pnum",
                 xformla = covariate, panel = TRUE, bstrap = TRUE, 
...
glance(vc.cls.p)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- vc.cls.p", "- att")) %>% put()
vc.cls.p %>% put()
warnings()

# w/ covariates, SE clustered at district level 
vc.cls.d <- att_gt(yname = outname[i], tname = "time", idname = "dnum",
                 gname = "group", control_group = "notyettreated",
                 base_period = "varying",  # varying reference point
                 est_method = "dr", clustervars = "dnum",
                 xformla = covariate, panel = TRUE, bstrap = TRUE, 
                 cband = TRUE, data = data_g9)
glance(vc.cls.d)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- vc.cls.d", "- att")) %>% put()
vc.cls.d %>% put()
warnings()
...
# w/ covariates, SE clustered at provincial level, no bootstrap
uc.cls.p.noboot <- att_gt(yname = outname[i], tname = "time", idname = "dnum",
                 gname = "group", control_group = "notyettreated",
                 base_period = "universal",  # fixed reference point
                 est_method = "dr", clustervars = "pnum",
                 xformla = covariate, panel = TRUE, bstrap = FALSE, 
                 cband = FALSE, data = data_g9)
glance(uc.cls.p.noboot)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- uc.cls.p.noboot", "- att")) %>% put()
uc.cls.p.noboot %>% put()
warnings()

# w/ covariates, SE clustered at provincial level
uc.cls.p <- att_gt(yname = outname[i], tname = "time", idname = "dnum",
                 gname = "group", control_group = "notyettreated",
                 base_period = "universal",  # fixed reference point
                 est_method = "dr", clustervars = "pnum",
                 xformla = covariate, panel = TRUE, bstrap = TRUE, 
...
glance(uc.cls.p)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- uc.cls.p", "- att")) %>% put()
uc.cls.p %>% put()
warnings()

# w/ covariates, SE clustered at district level
uc.cls.d <- att_gt(yname = outname[i], tname = "time", idname = "dnum",
                 gname = "group", control_group = "notyettreated",
                 base_period = "universal",  # fixed reference point
                 est_method = "dr", clustervars = "dnum",
                 xformla = covariate, panel = TRUE, bstrap = TRUE, 
...
glance(uc.cls.d)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- uc.cls.d", "- att")) %>% put()
uc.cls.d %>% put()
warnings()
...
listv <- c("v", "v.cls", 
           "vc", "vc.cls.p.noboot", "vc.cls.p", "vc.cls.d", "vc.clslast", "vc.clsant1")
attgt_list_v <- lapply(listv, get)

listu <- c("u", "u.cls", 
           "uc", "uc.cls.p.noboot", "uc.cls.p", "uc.cls.d", "uc.clslast", "uc.clsant1")
...
attgt_tidy_v_frame <- bind_rows(attgt_tidy_v)
...
attgt_tidy_u_frame <- bind_rows(attgt_tidy_u)
...
aggte_sim_v_frame <- bind_rows(aggte_sim_v)
...
aggte_grp_v_frame <- bind_rows(aggte_grp_v)
...
aggte_dyn_v_frame <- bind_rows(aggte_dyn_v)
...
aggte_dyn_v_frame_b <- bind_rows(aggte_dyn_v_b)
...
aggte_sim_u_frame <- bind_rows(aggte_sim_u)
...
aggte_grp_u_frame <- bind_rows(aggte_grp_u)
preevent$group  <- as.character(preevent$group)
...
aggte_dyn_u_frame <- bind_rows(aggte_dyn_u)
...
aggte_dyn_u_frame_b <- bind_rows(aggte_dyn_u_b)
...
# In case of 
# lazy-load database '/Library/Frameworks/R.framework/Versions/4.4-arm64/Resources/library/HonestDiD/R/HonestDiD.rdb' is corrupt -> restart the R session, and re-run.
# .rs.restartR()
...
honest_specs <- c("uc.cls.p") #, "uc.cls.d")

for (k in which(listu %in% honest_specs)) {
 ...
 set.seed(55555)
 sm <- honest_did_cluster_boot_avg(es = dyn_u[[k]], type = "smoothness",
                                   Mvec = seq(0, 20, by = 2))
 ...
 sm_u <- bind_rows(sm_rbci, sm_orgci) %>%
   ...
   mutate(xaxis = "M",
          ...
          name = listu[k],
          outcome = outname[i],
          point_est = sm$point_est)
 sm_u_list[[k]] <- sm_u
 
 set.seed(55555)
 rm <- honest_did_cluster_boot_avg(
   es = dyn_u[[k]], type = "relative_magnitude",
   Mbarvec = seq(0, 1.5, by = 0.1),
   grid.ub = unname(rm_grid_ub[outname[i]]))
 ...
 rm_u <- bind_rows(rm_rbci, rm_orgci) %>%
   ...
   mutate(xaxis = "Mbar",
          ...
          name = listu[k],
          outcome = outname[i],
          point_est = rm$point_est)
 rm_u_list[[k]] <- rm_u
 
 
 # --------------------------------------------------------------------------
 # Treatment effect at e = 0
 # --------------------------------------------------------------------------
 
 set.seed(55555)
 sm_e0 <- honest_did_cluster_boot(
   es = dyn_u[[k]], e = 0, 
   type = "smoothness",
   Mvec = seq(0, 20, by = 2))
 sm_e0_rbci <- as.data.frame(sm_e0$robust_ci)
 sm_e0_orgci <- as.data.frame(sm_e0$orig_ci)
 
 colnames(sm_e0_orgci) <- c("lb", "ub", "method", "Delta")
 sm_e0_orgci$M <- NA
 
 sm_e0_u <- bind_rows(sm_e0_rbci, sm_e0_orgci) %>%
   setNames(c("lb", "ub", "method", "Delta", "M")) %>%
   mutate(xaxis = "M",
          type = sm_e0$type,
          estimand = "e0",
          name = listu[k],
          outcome = outname[i],
          point_est = sm_e0$point_est)
 sm_e0_u_list[[k]] <- sm_e0_u
 
 set.seed(55555)
 rm_e0 <- honest_did_cluster_boot(
   es = dyn_u[[k]], e = 0, 
   type = "relative_magnitude",
   Mbarvec = seq(0, 1.5, by = 0.1),
   grid.ub = unname(rm_grid_ub[outname[i]]))
 rm_e0$robust_ci <- rm_e0$robust_ci[-1, ]  # Drop 0 as it is not really allowed.
 rm_e0_rbci <- as.data.frame(rm_e0$robust_ci)
 rm_e0_orgci <- as.data.frame(rm_e0$orig_ci)
 
 colnames(rm_e0_orgci) <- c("lb", "ub", "method", "Delta")
 rm_e0_orgci$Mbar <- NA
 
 rm_e0_u <- bind_rows(rm_e0_rbci, rm_e0_orgci) %>%
   setNames(c("lb", "ub", "method", "Delta", "M")) %>%
   mutate(xaxis = "Mbar",
          type = rm_e0$type,
          estimand = "e0",
          name = listu[k],
          outcome = outname[i],
          point_est = rm_e0$point_est)
 rm_e0_u_list[[k]] <- rm_e0_u
}

sm_u_frame <- bind_rows(sm_u_list)
rm_u_frame <- bind_rows(rm_u_list)

sm_e0_u_frame <- bind_rows(sm_e0_u_list)
rm_e0_u_frame <- bind_rows(rm_e0_u_list)

didsensitivity <- rbind(
  sm_u_frame,
  rm_u_frame,
  sm_e0_u_frame,
  rm_e0_u_frame
...
if(!dir.exists("./output")) {dir.create("./output")}
if(!dir.exists("./output/q_disl")) {dir.create("./output/q_disl")}
if(!dir.exists("./output/q_disl/placebo")) {dir.create("./output/q_disl/placebo")}
outpath <- c("./output/q_disl/placebo/")
...
data_g9_1_all <- data_g9 %>%
  mutate(
    group = case_when(
      group_org == 4 ~ group_org - 1,  # 1 lead
      group_org == 5 ~ group_org - 1,  # 1 lead
      group_org == 6 ~ group_org - 1,  # 1 leads
      group_org == 8 ~ group_org - 1,  # 1 leads
      TRUE ~ group_org),
    event = ifelse(group_org >= 4, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))

data_g9_1_late <- data_g9 %>%
  mutate(
    group = case_when(
      group_org == 5 ~ group_org - 1,  # 1 lead
      group_org == 6 ~ group_org - 1,  # 1 leads
      group_org == 8 ~ group_org - 1,  # 1 leads
      TRUE ~ group_org),
    event = ifelse(group_org >= 5, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))

data_g9_1_late_no4 <- data_g9 %>%
  filter(group_org != 4) %>%
  mutate(
    group = case_when(
      group_org == 5 ~ group_org - 1,  # 1 lead
...
data_g9_2_all <- data_g9 %>%
  mutate(
    group = case_when(
      group_org == 4 ~ group_org - 2,  # 2 lead
      group_org == 5 ~ group_org - 2,  # 2 lead
      group_org == 6 ~ group_org - 2,  # 2 leads
      group_org == 8 ~ group_org - 2,  # 2 leads
      TRUE ~ group_org),
    event = ifelse(group_org >= 4, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))

data_g9_2_late <- data_g9 %>%
  mutate(
    group = case_when(
      group_org == 5 ~ group_org - 1,  # 1 lead
      group_org == 6 ~ group_org - 2,  # 2 leads
...
data_g9_3 <- data_g9 %>%
  mutate(
    group = case_when(
      group_org == 5 ~ group_org - 1,  # 1 lead
      group_org == 6 ~ group_org - 2,  # 2 leads
      group_org == 8 ~ group_org - 3,  # 3 leads
      TRUE ~ group_org),
    event = ifelse(group_org >= 5, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))

data_g9_3_late_no4 <- data_g9 %>%
  filter(group_org != 4) %>%
  mutate(
    group = case_when(
      group_org == 5 ~ group_org - 1,  # 1 lead
...
data_g5_1 <- data_g9 %>%
  filter(group_org != 4) %>%
  mutate(
    group = ifelse(group_org == 5, group_org - 1, group_org),
    event = ifelse(group_org >= 5, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))

data_g6_1 <- data_g9 %>%
  filter(!group_org %in% c(4,5)) %>%
  mutate(
    group = ifelse(group_org == 6, group_org - 1, group_org),
    event = ifelse(group_org >= 5, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))
...
data_g6_2 <- data_g9 %>%
  filter(!group_org %in% c(4,5)) %>%
  mutate(
    group = ifelse(group_org == 6, group_org - 2, group_org),
    event = ifelse(group_org >= 5, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))

data_g8_1 <- data_g9 %>%
  filter(!group_org %in% c(4,5,6)) %>%
  mutate(
    group = ifelse(group_org == 8, group_org - 1, group_org),
    event = ifelse(group_org >= 5, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))
...
data_g8_2 <- data_g9 %>%
  filter(!group_org %in% c(4,5,6)) %>%
  mutate(
    group = ifelse(group_org == 8, group_org - 2, group_org),
    event = ifelse(group_org >= 5, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))
...
data_g8_3 <- data_g9 %>%
  filter(!group_org %in% c(4,5,6)) %>%
  mutate(
    group = ifelse(group_org == 8, group_org - 3, group_org),
    event = ifelse(group_org >= 5, time - group, event_org),
    treat = ifelse(time >= group & group > 0, 1, 0))
...
data_list <- list(
  g9_no5 = data_g9_no5,
  g9_1_all = data_g9_1_all,
  g9_1_late = data_g9_1_late,
  g9_1_late_no4 = data_g9_1_late_no4,
  g9_2_all = data_g9_2_all,
  g9_2_late = data_g9_2_late,
  g9_2_late_no4 = data_g9_2_late_no4,
  g9_3 = data_g9_3,
  g9_3_late_no4 = data_g9_3_late_no4,
  g5_1 = data_g5_1,
  g6_1 = data_g6_1,
  g6_2 = data_g6_2,
  g8_1 = data_g8_1,
...
vc.cls.p <- att_gt(yname = outname[i], tname = "time", idname = "dnum",
                 ...
                 est_method = "dr", clustervars = "pnum",
                 xformla = covariate, panel = TRUE, bstrap = TRUE, 
                 cband = TRUE, data = data_list[[name]])
glance(vc.cls.p)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- vc.cls.p", "- att")) %>% put()
vc.cls.p %>% put()
...
listv <- c("vc.cls.p", "vc.cls.d", "vc.clslast")
attgt_list_v <- lapply(listv, get)

listu <- c("uc.cls.p", "uc.cls.d", "uc.clslast")
attgt_list_u <- lapply(listu, get)
...
u.group <- unique(data_list[[name]]$group) # unique group
u.group <- sort(u.group)[-1] # remove untreated
n.group <- sapply(u.group, function(gg) nrow(subset(data_list[[name]], group == gg))) # number of group
ngroup.mat <- as_tibble(cbind(u.group, n.group))
colnames(ngroup.mat) <- c("group", "n.group")
...
write_xlsx(attgt_tidy_frame,
           paste0(outpath, outname[i], "_attgt_", name, ".xlsx"))
...
write_xlsx(aggte_sim_frame,
           paste0(outpath, outname[i], "_aggte_sim_", name,".xlsx"))

write_xlsx(aggte_grp_frame,
           paste0(outpath, outname[i], "_aggte_grp_", name, ".xlsx"))

write_xlsx(aggte_dyn_frame,
           paste0(outpath, outname[i], "_aggte_dyn_", name,".xlsx"))
...
# A3. Subgroup analyses: district-level poverty status
<br>
```{r poverty}
...
table1_pov <- data %>%
  ...
  dplyr::select(group, facnum, imps, litr, semp, urba, pov_med) %>%
  mutate(across(imps:urba, ~.x * 100)) %>%
  mutate(pov_med_label = ifelse(pov_med == 1, "High-poverty", "Low-poverty")) %>%
  dplyr::select(-pov_med) %>%
  tbl_strata(strata = pov_med_label,
             ~tbl_summary(.x,
                          by = group,
                          statistic = list(
                            facnum ~ "{sum}",
...
table1_pov
show_header_names(table1_pov) %>% put()
...
groups <- list(
  overall = data_pov,
  poor = subset(data_pov, pov_med == 1), # high-poverty
  rich = subset(data_pov, pov_med == 0)  # low-poverty
)
...
for (g in names(groups)) {
  ...
  for (i in 1:length(outname)) {
    ...
    preevent <- dat %>%
      filter(group > 0 & event == -1) %>%
      mutate(avgunit = mean(!!outn, na.rm = TRUE)) %>%
      group_by(group) %>%
      mutate(avggrp = mean(!!outn, na.rm = TRUE)) %>%
      ungroup() %>%
      dplyr::select(c("group", "avgunit", "avggrp")) %>%
      unique()
    
    u.group <- unique(dat$group)
    u.group <- sort(u.group)[-1]
    
    n.group <- sapply(u.group, function(gg) nrow(subset(dat, group == gg)))
    ngroup.mat <- as_tibble(cbind(u.group, n.group))
    colnames(ngroup.mat) <- c("group", "n.group")
    ...
    preevent <- preevent %>%
      left_join(ngroup.mat, by = "group") %>%
      mutate(
        weight = n.group / sum(n.group),
        avgdyn = sum(avggrp * weight))
    
    set.seed(55555)
    
    cs_g9_nyt <- att_gt(
      yname = outname[i], tname = "time", idname = "dnum", gname = "group",
      control_group = "notyettreated", base_period = "universal", 
      est_method = "dr", clustervars = "pnum",
      #      xformla = covariate,
      panel = TRUE, bstrap = TRUE, cband = TRUE, data = dat)
    
    dyn_obj <- aggte(cs_g9_nyt, type = "dynamic", na.rm = TRUE)
    
    dyn_frame <- tidy(dyn_obj) %>%
      mutate(
        overall.att = dyn_obj$overall.att,
        overall.se = dyn_obj$overall.se,
        overall.conf.low = overall.att - qnorm(0.975) * overall.se,
        overall.conf.high = overall.att + qnorm(0.975) * overall.se,
        overall.change = overall.att / unique(preevent$avgdyn),
        group = ifelse(event.time < 0, "Pre-NHI", "Post-NHI"))
    
    dyn_frame <- dyn_frame %>%
      mutate(
        avgunit = unique(preevent$avgunit),
        avgdyn = unique(preevent$avgdyn),
        change = estimate / avgdyn,
        outcome = outname[i],
        pretrendtype = "fixed",
    ...
    cs_dyn_list[[length(cs_dyn_list) + 1]] <- dyn_frame
  }}

cs_dyn_frame <- bind_rows(cs_dyn_list)
warnings()

write_xlsx(cs_dyn_frame, "./output/q_disl/did_poverty_CS_nocov.xlsx")
...
data_pov <- data %>%
  ...
  filter(quarter >= 2015.4 & quarter <= 2017.3) %>%
  mutate(event = ifelse(group == max(group), -100, event)) %>%  # Last group as control
  mutate(group = ifelse(group == max(group), 0, group)) %>%
  mutate(group_d = ifelse(group == max(group), 0, 1))  # Ever-treated 1, Never-treated 0
...
saiw_list <- bind_rows(unlist(list(
  overall_list, poor_list, rich_list),
  recursive = FALSE))
saiw_att_list <- bind_rows(unlist(
  list(overall_att_list, poor_att_list, rich_att_list),
  recursive = FALSE))

saiw_att_list <- saiw_att_list %>%
  dplyr::select(-statistic) %>%
  mutate(overall.conf.low = estimate - (qnorm(0.975) * std.error),
         overall.conf.high = estimate + (qnorm(0.975) * std.error)) %>%
  rename( # new <- old
...
write_xlsx(saiw_tidy_frame, "./output/q_disl/did_poverty_AS.xlsx")
...
for (i in 1:length(outname)) {
  ...
  overall_att <- data_pov %>% 
    do(broom::tidy(summary(
      feols(!!outn ~ i(time, facnum) + i(time, semp) + i(time, urba) + i(time, litr) + 
              i(time, imps) +
              sunab(group, time) | dnum + time, cluster = ~pnum, data = .), 
      agg = "att"))) %>%
    filter(term %in% c("ATT")) %>%
    mutate(outcome = outname[i],
           pgroup = "overall",
           avgpov = prevent_all,
           overall.change = estimate / avgpov)
...
saiw_list <- bind_rows(unlist(
  list(overall_list, poor_list, rich_list),
  recursive = FALSE))
saiw_att_list <- bind_rows(unlist(
  list(overall_att_list, poor_att_list, rich_att_list), 
  recursive = FALSE))
...
write_xlsx(saiw_tidy_frame, "./output/q_disl/did_poverty_AS_time-interaction.xlsx")

```

# A6. Robustness check: DiD heterogeneity-robust estimators
<br>
```{r hetero}
...
# ref = c(-1, -1000) means excluding the period just before the treatment (event == -1) 
...
twfe_frame <- bind_rows(twfe_list) %>% 
  print(n = Inf)
...
stacked_frame <- bind_rows(stacked_list) %>% 
  print(n = Inf)
...
impt_gdn_frame <- bind_rows(impt_gdn_list) %>% 
  print(n = Inf)
...
for (i in seq_along(outname)) {
  etwfe_list[[i]] <- data_alt %>%
      dplyr::select(all_of(outname[i]), 
                    facnum_fixed, semp, urba, litr, imps, time, group, pnum, dnum) %>%
    rename(wgroup = group) %>%
    etwfe(
...
etwfe_frame <- bind_rows(etwfe_dyn_list) %>% 
  print(n = Inf)
...
write_xlsx(didalt, "./output/fig_a_06_did_hetero.xlsx")

```

# A9. Robustness check: Controlled interrupted time series analysis (quarterly)
<br>
```{r cits}
...
for (j in pnumlist) {
  ...
  its_set <- bind_rows(its_df)
  its_set$pnum <- j
  its_m1q[[j]] <- its_set
}

its_m1q_all <- bind_rows(its_m1q)
...
for (j in 1:length(its_all)) {
  for (i in 1:length(outname)) {
    ...
    colnames(b5_summary) <- c("estimate", "conf.low", "conf.high", "statistic", 
                              "p.value", "label")
    ...
    b6_summary <- cbind.data.frame(its_b6$TE.random,
                                   ...
                                   its_b6$pval.random,
                                   "DiD in level (beta6)")
    colnames(b6_summary) <- c("estimate", "conf.low", "conf.high", "statistic", 
                              "p.value", "label")
    ...
    b7_summary <- cbind.data.frame(its_b7$TE.random,
                                   ...
                                   its_b7$pval.random,
                                   "DiD in slope (beta7)")
    colnames(b7_summary) <- c("estimate", "conf.low", "conf.high", "statistic", 
                              "p.value", "label")
  ...
  meta_temp <- bind_rows(meta_summary)
  meta_temp$dataset <- meta_listname[j]
  meta_summary_df[[j]] <- meta_temp
}

its_meta_final_q <- bind_rows(meta_summary_df) 
# %>%
